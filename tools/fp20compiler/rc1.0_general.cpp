...
static void GenerateInput(int portion, char variable, MappedRegisterStruct reg) {
    ...
    printf("MASK(NV097_SET_COMBINER_%s_ICW_%c_SOURCE, %s)", portion_s, variable, get_register_name_string(reg.reg.bits.name));
...
void GeneralFunctionStruct::Invoke(int stage, int portion, BiasScaleEnum bs)
{
    ...
    printf("    MASK(NV097_SET_COMBINER_%s_OCW_AB_DST, %s)\n", portion_s, get_register_name_string(op[0].reg[0].reg.bits.name));
    printf("    | MASK(NV097_SET_COMBINER_%s_OCW_CD_DST, %s)\n", portion_s, get_register_name_string(op[1].reg[0].reg.bits.name));
    printf("    | MASK(NV097_SET_COMBINER_%s_OCW_SUM_DST, %s)\n", portion_s, get_register_name_string(op[2].reg[0].reg.bits.name));
